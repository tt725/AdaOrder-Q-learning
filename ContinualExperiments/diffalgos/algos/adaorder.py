...
class AdaOrderAgent(object):
    ...
    def get_adaorder_q_target_no_grad(self, obs_next_tensor, rews_tensor, done_tensor):
        sample_idxs = np.random.choice(self.num_C, self.num_M, replace=False)
        with torch.no_grad():
            a_tilda_next, _, _, log_prob_a_tilda_next, _, _ = self.policy_net.forward(obs_next_tensor)

            q_prediction_next_list_all = []
            for q_i in range(self.num_C):
                q_prediction_next_all = self.q_target_net_list[q_i](torch.cat([obs_next_tensor, a_tilda_next], 1))
                q_prediction_next_list_all.append(q_prediction_next_all)
            q_prediction_next_all_ave = torch.cat(q_prediction_next_list_all, 1).mean(dim=1).reshape(-1, 1)
            ...
            q_prediction_next_sort, _ = torch.sort(q_prediction_next_cat, dim=1)

            q_adaorder = Tensor(np.random.uniform(0, 1, q_prediction_next_all_ave.shape)).to(device=self.device)
            for i in range(len(q_prediction_next_all_ave)):
                if q_prediction_next_sort[i][0] >= q_prediction_next_all_ave[i][0]:
                    q_adaorder[i][0] = q_prediction_next_sort[i][0]
                elif q_prediction_next_sort[i][self.num_M - 1] <= q_prediction_next_all_ave[i][0]:
                    q_adaorder[i][0] = q_prediction_next_sort[i][self.num_M - 1]
                else:
                    for j in range(self.num_M - 1):
                        if q_prediction_next_sort[i][j] < q_prediction_next_all_ave[i][0] and q_prediction_next_sort[i][
                            j + 1] > q_prediction_next_all_ave[i][0]:
                            q_adaorder[i][0] = q_prediction_next_sort[i][j]
                            break

            next_q_with_log_prob = q_adaorder - self.alpha * log_prob_a_tilda_next
    ...
    def train(self, logger):
        ...
        for i_update in range(num_update):
            ...
            """Q loss"""
            y_q, sample_idxs = self.get_adaorder_q_target_no_grad(obs_next_tensor, rews_tensor, done_tensor)
            q_prediction_list = []
            for q_i in range(self.num_C):
                q_prediction = self.q_net_list[q_i](torch.cat([obs_tensor, acts_tensor], 1))
                q_prediction_list.append(q_prediction)
            q_prediction_cat = torch.cat(q_prediction_list, dim=1)
            y_q = y_q.expand((-1, self.num_C)) if y_q.shape[1] == 1 else y_q
            q_loss_all = self.mse_criterion(q_prediction_cat, y_q) * self.num_C
            ...
            if ((i_update + 1) % self.policy_update_delay == 0) or i_update == num_update - 1:
                ...
                q_a_tilda_list = []
                for sample_idx in range(self.num_C):
            ...
            if i_update == num_update - 1:
                logger.store(LossPi=policy_loss.cpu().item(), LossQ1=q_loss_all.cpu().item() / self.num_C,
