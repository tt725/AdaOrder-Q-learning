...
class OrderAgent(object):
    def __init__(self, env_name, obs_dim, act_dim, act_limit, device,
                 ...
                 utd_ratio=20, num_Q=10, order_m=2,
        ...
        self.num_Q = num_Q
        self.order_m = order_m #changed by tantao
    ...
    def get_order_q_target_no_grad(self, obs_next_tensor, rews_tensor, done_tensor):
        ...
        with torch.no_grad():
            ...
            m = self.order_m
            kth_q, kth_indices = torch.kthvalue(q_prediction_next_cat, k=m, dim=1, keepdim=True)
            next_q_with_log_prob = kth_q - self.alpha * log_prob_a_tilda_next
    ...
    def train(self, logger):
        ...
        for i_update in range(num_update):
            ...
            """Q loss"""
            y_q, sample_idxs = self.get_order_q_target_no_grad(obs_next_tensor, rews_tensor, done_tensor)
            q_prediction_list = []
            for q_i in range(self.num_Q):
                q_prediction = self.q_net_list[q_i](torch.cat([obs_tensor, acts_tensor], 1))
                q_prediction_list.append(q_prediction)
