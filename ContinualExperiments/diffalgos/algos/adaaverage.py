...
class AdaAverageAgent(object):
    ...
    def get_adaaverage_q_target_no_grad(self, obs_next_tensor, rews_tensor, done_tensor):
        ...
        with torch.no_grad():
            ...
            q_adaaverage = q_prediction_next_cat.mean(dim=1).reshape(-1, 1)
            next_q_with_log_prob = q_adaaverage - self.alpha * log_prob_a_tilda_next
    ...
    def train(self, logger):
        ...
        for i_update in range(num_update):
            ...
            """Q loss"""
            y_q, sample_idxs = self.get_adaaverage_q_target_no_grad(obs_next_tensor, rews_tensor, done_tensor)
            q_prediction_list = []
            for q_i in range(self.num_C):
                q_prediction = self.q_net_list[q_i](torch.cat([obs_tensor, acts_tensor], 1))
                q_prediction_list.append(q_prediction)
