...
class OrderQLearner:
    def __init__(self, number_estimator, number_order, epsilon=1.0, gamma=0.95, learningRate=0.1):
    ...
    def learning(self, state, action, reward, next_state, done):
        ...
        self.Q[index][state[0]][state[1]][action] += self.learningRate * (Y - self.Q[index][state[0]][state[1]][action])
