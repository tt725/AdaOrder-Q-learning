...
import time
from Env import Env
from AdaMaxminQLearner import AdaMaxminQLearner

NUMBER_ESTIMATOR = 8
PARAMETER = 0.1

def AdaMaxminQLearning():
    max_Q_S_repeat = np.zeros((1000, 100))
    for repeat in range(1000):
        env = Env()
        agent = AdaMaxminQLearner(number_estimator=NUMBER_ESTIMATOR, parameter=PARAMETER)
        max_Q_S= AdaMaxminQUpdate(env=env, agent=agent)
...
def AdaMaxminQUpdate(env, agent):
...
if __name__ == "__main__":
    log = Logger(
        './Result/log.' + "AdaMaxminQ(" + str(NUMBER_ESTIMATOR) + "," + str(PARAMETER) + ")"
