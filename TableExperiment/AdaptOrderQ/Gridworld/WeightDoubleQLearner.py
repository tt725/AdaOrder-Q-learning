...
class WeightDoubleQLearner:
    ...
    def learning(self, state, action, reward, next_state, done):
        ...
        if np.random.random() >= 0.5:
            if not done:
                A1 = np.argmax(self.Q1[next_state[0]][next_state[1]][:])
                Y += self.gamma * (0.5 * self.Q1[next_state[0]][next_state[1]][A1] + 0.5 *
                                   self.Q2[next_state[0]][next_state[1]][A1])
            self.Count_S_A_1[state[0]][state[1]][action] += 1
            lr_1 = self.learningRate / np.power(self.Count_S_A_1[state[0]][state[1]][action], 0.8)
            self.Q1[state[0]][state[1]][action] += lr_1 * (Y - self.Q1[state[0]][state[1]][action])
        else:
            if not done:
                A2 = np.argmax(self.Q2[next_state[0]][next_state[1]][:])
                Y += self.gamma * (0.5 * self.Q1[next_state[0]][next_state[1]][A2] + 0.5 *
                                   self.Q2[next_state[0]][next_state[1]][A2])
            self.Count_S_A_2[state[0]][state[1]][action] += 1
            lr_2 = self.learningRate / np.power(self.Count_S_A_2[state[0]][state[1]][action], 0.8)
